...
# Deploy DeepSeek V3 with P/D disaggregation using llm-d
...
echo "=========================================="
echo "Deploying DeepSeek V3.1 P/D Disaggregation"
echo "=========================================="
...
if kubectl get svc ms-deepseek-v3-llm-d-modelservice-prefill -n "$NAMESPACE" &>/dev/null; then
...
# Check HTTPRoute (created by Helm with httpRoute.create: true)
if kubectl get httproute -n "$NAMESPACE" &>/dev/null 2>&1; then
    echo "✓ HTTPRoute created by Helm"
else
    echo "⚠ Warning: HTTPRoute not found (should be created by Helm)"
fi
...
echo "  - 1 prefill pod with TP=8 (1/1 ready)"
echo "  - 1 decode pod with TP=8 (2/2 ready)"
echo "  - 1 GAIE EPP pod (1/1 ready)"
echo "  - 1 gateway pod (1/1 ready)"
echo "  - 2 backend services (prefill, decode)"
echo "  - 1 InferencePool (gaie-deepseek-v3)"
echo "  - 1 HTTPRoute"
echo ""
echo "Deployment may take 10-15 minutes for first time (large model download ~700GB)"
echo "Subsequent deployments take ~3-5 minutes (model cached)"
echo ""
