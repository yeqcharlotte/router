...
# Run benchmark for Llama 3.1 with llm-d (GAIE P-D disaggregation)
...
echo "This may take 5-10 minutes on first deployment (model download)"
echo ""
...
while true; do
    ...
    GAIE_READY=$(kubectl get pods -n "$NAMESPACE" -l inferencepool=gaie-llama31-epp -o json 2>/dev/null | jq -r '[.items[] | select(.status.phase=="Running" and (.status.containerStatuses[]? | select(.ready==true)))] | length' || echo "0")

    echo "  Prefill pods: $PREFILL_READY/$EXPECTED_PREFILL ready"
    echo "  Decode pods: $DECODE_READY/$EXPECTED_DECODE ready"
...
if ! kubectl get inferencepool.inference.networking.x-k8s.io gaie-llama31 -n "$NAMESPACE" &>/dev/null; then
    echo "❌ Error: InferencePool 'gaie-llama31' not found"
    exit 1
fi
...
if [[ "$TEST_RESULT" == "FAILED" ]] || [[ ! "$TEST_RESULT" =~ "choices" ]]; then
    ...
    echo "  kubectl logs -n $NAMESPACE -l inferencepool=gaie-llama31-epp --tail=50"
    echo "  kubectl describe inferencepool gaie-llama31 -n $NAMESPACE"
    exit 1
fi
...
echo "  Architecture: 8 prefill pods + 8 decode pods (1 GPU each)"
echo "  Concurrency: $MAX_CONCURRENCY"
