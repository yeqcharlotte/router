        ...
        try:
            ...
            for comment in comments:
                if comment.get("user", {}).get(
                    "login"
                ) == "github-actions[bot]" and "vLLM Router Benchmark Results" in comment.get(
                    "body", ""
                ):
                    return comment["id"]
...
def main():
    ...
    parser.add_argument(
        "--repo-owner", type=str, default="vllm-project", help="GitHub repository owner"
    )
    parser.add_argument(
        "--repo-name", type=str, default="vllm", help="GitHub repository name"
    )
