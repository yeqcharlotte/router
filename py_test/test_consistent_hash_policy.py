...
class ConsistentHashTester:
    ...
    def test_session_consistency(self, num_requests: int = 10) -> bool:
        ...
        self.log(
            f"✅ Session consistency test passed: {num_requests} requests with session_id '{session_id}' completed successfully"
        )
        return True
    ...
    def test_distribution_across_workers(self, num_sessions: int = 20) -> bool:
        ...
        if success_rate >= 0.8:  # Allow for some failures
            self.log(
                f"✅ Distribution test passed: {successful_requests}/{num_sessions} sessions succeeded ({success_rate:.1%})"
            )
            return True
        else:
            self.log(
                f"❌ Distribution test failed: Only {successful_requests}/{num_sessions} sessions succeeded ({success_rate:.1%})",
                "ERROR",
            )
            return False
    ...
    def test_concurrent_requests(self, num_concurrent: int = 10) -> bool:
        ...
        if success_rate >= 0.8:  # Allow for some failures in concurrent scenario
            self.log(
                f"✅ Concurrent test passed: {successful}/{num_concurrent} requests succeeded ({success_rate:.1%}) in {end_time - start_time:.2f}s"
            )
            return True
        else:
            self.log(
                f"❌ Concurrent test failed: Only {successful}/{num_concurrent} requests succeeded ({success_rate:.1%})",
                "ERROR",
            )
            return False
